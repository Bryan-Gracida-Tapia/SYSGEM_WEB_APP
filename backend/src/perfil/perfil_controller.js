...
const PerfilController = {
    ...
    async obtenerHistorialCargos(userId) {
        ...
        // Cambiamos la consulta agregando el JOIN con cargos
        // Usamos AS cargo para mantener el mismo nombre de propiedad en JavaScript
        const sqlHistorial = `
            SELECT
                cc.id,
                ca.nombre AS cargo,
                cc.year
            FROM comunero_cargos_cumplidos cc
                     JOIN cargos ca ON cc.cargo_id = ca.id
            WHERE cc.comunero_id = ?
